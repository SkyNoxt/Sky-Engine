...
typename BVH<T>::BVHNode* BVH<T>::recurse(std::vector<BVHBox>& workSet, unsigned int depth)
{
	...
	for(unsigned int i = 0; i < 3; ++i)
	{
		...
		for(T testSplit = start + step; testSplit < stop - step; testSplit += step)
		{
			...
			for(unsigned int j = 0; j < workSet.size(); ++j)
			{
				...
				T value;
				if(axis == 0) value = box.center.x;
				else if(axis == 1) value = box.center.y;
				else value = box.center.z;
	...
	for(unsigned int i = 0; i < workSet.size(); ++i)
	{
		...
		T value;
		if(bestAxis == 0) value = box.center.x;
		else if(bestAxis == 1) value = box.center.y;
		else value = box.center.z;
		...
		if(node->isLeaf())
		{
			...
			for(unsigned int i = 0; i < leaf->numIndices; i+=3)
			{
				if(mesh->triangleIntersect(ray,
					...
					mesh->vertexArray[leaf->indexArray[i + 2]].position, tempDist, u, v) && tempDist > 0 && tempDist < distance)
					{
						distance = tempDist;
						barycenter = { u, v };
